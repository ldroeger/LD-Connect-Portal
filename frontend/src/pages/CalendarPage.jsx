...
export default function CalendarPage() {
  ...
  const eventStyleGetter = (event) => {
    const color = labelColor(event.label, labels)
    return {
      style: {
  ...
  const uniqueLabels = [...new Set(events.map(e => e.label).filter(Boolean))]
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, height: '100%' }}>
      ...
      {uniqueLabels.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {uniqueLabels.map(label => {
            ...
            return (
              <span key={label} style={{ display: 'inline-flex', alignItems: 'center', padding: '3px 10px', borderRadius: 20, fontSize: '0.75rem', fontWeight: 600, background: color, color: getTextColor(color) }}>
                {label}
              </span>
            )
          })}
      ...
      {selected && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 16, padding: 24, maxWidth: 440, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}
            onClick={e => e.stopPropagation()}>
            <div style={{ height: 4, borderRadius: 4, background: labelColor(selected.label, labels), marginBottom: 16 }} />
            <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: 12 }}>{selected.title}</h3>
            {selected.label && (
              <div style={{ marginBottom: 10 }}>
                <span style={{ padding: '3px 10px', borderRadius: 12, fontSize: '0.75rem', fontWeight: 600, background: labelColor(selected.label, labels), color: getTextColor(labelColor(selected.label, labels)) }}>
                  {selected.label}
                </span>
              </div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: '0.86rem', color: 'var(--text-2)' }}>
              <div style={{ display: 'flex', gap: 12 }}><span style={{ color: 'var(--text-3)', width: 60 }}>Beginn</span><span>{fmtDt(selected.start)}</span></div>
              <div style={{ display: 'flex', gap: 12 }}><span style={{ color: 'var(--text-3)', width: 60 }}>Ende</span><span>{fmtDt(selected.end)}</span></div>
              {selected.location && <div style={{ display: 'flex', gap: 12 }}><span style={{ color: 'var(--text-3)', width: 60 }}>Ort</span><span>{selected.location}</span></div>}
              {selected.description && <div style={{ display: 'flex', gap: 12 }}><span style={{ color: 'var(--text-3)', width: 60 }}>Notiz</span><span>{selected.description}</span></div>}
            </div>
            <button style={{ marginTop: 16, padding: '9px 24px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--surface-2)', color: 'var(--text)', cursor: 'pointer', fontSize: '0.88rem', width: '100%' }}
              onClick={() => setSelected(null)}>Schließen</button>
          </div>
        </div>
      )}
    </div>
