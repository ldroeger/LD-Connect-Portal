...
router.get('/', adminMiddleware, (_req, res) =>
  res.json({ users: localDb.db.prepare(
    'SELECT id,email,name,powerbird_id,role,is_active,feature_calendar,feature_vacation,feature_hours,feature_news_read,feature_news_write,feature_todos_read,feature_todos_create,feature_tools,feature_tools_search,feature_show_verleih,created_at FROM users ORDER BY name'
  ).all() })
);
...
router.put('/:id', adminMiddleware, (req, res) => {
  try {
    const { name, powerbird_id, role, is_active, feature_calendar, feature_vacation, feature_hours, feature_news_read, feature_news_write, feature_todos_read, feature_todos_create, feature_tools, feature_tools_search, feature_show_verleih } = req.body;
    localDb.db.prepare(`
      UPDATE users SET
        name = COALESCE(?, name),
